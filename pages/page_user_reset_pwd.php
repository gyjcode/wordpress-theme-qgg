...
<?php
the_module_loader('module_get_page_user_reset_pwd', false);
	...
	case 'resetpassword' :
		...
		if ( is_wp_error($user) ) {
			if ( $user->get_error_code() === 'expired_key' ){
				wp_redirect( module_get_page_user_reset_pwd() . '?&action=lostpassword&error=expiredkey' );
			}
			else{
				wp_redirect( module_get_page_user_reset_pwd() . '?&action=lostpassword&error=invalidkey' );
			}
			exit;
		}
		...
		if ( ( ! $errors->get_error_code() ) && isset( $_POST['pass1'] ) && !empty( $_POST['pass1'] ) ) {
			...
			wp_redirect( module_get_page_user_reset_pwd() . '?&action=success' );
			exit;
		}
...
function retrieve_password() {
	...
	$message .= network_site_url(module_get_page_user_reset_pwd() . "?&action=resetpassword&key=$key&login=" . rawurlencode($user_login), 'login');
...
<div class="content">
	<div class="main page-resetpassword">
		...
		if( $lost_active ){ 
			if( $errors !== true ){
		?>
		<form action="<?php echo esc_url( module_get_page_user_reset_pwd() . '?&action=lostpassword' ); ?>" method="post">
