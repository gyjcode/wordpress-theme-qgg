	...
	<div class="pagewrapper clearfix" id="profile" >
		...
		<aside class="pagesidebar" style="top:44px;">
			<ul class="pagesider-menu">
				<li <?php if((isset($_GET["pd"]) && $_GET["pd"]=='info') || !isset($_GET["pd"])){?>class="active"<?php }?> ><a href="?pd=info">个人信息</a></li>
				<li <?php if(isset($_GET["pd"]) && $_GET["pd"]=='money'){?>class="active"<?php }?> ><a href="?pd=money">我的资产</a></li>
				<li <?php if(isset($_GET["pd"]) && $_GET["pd"]=='cart'){?>class="active"<?php }?> ><a href="?pd=cart">下载清单</a></li>
				<li <?php if(isset($_GET["pd"]) && $_GET["pd"]=='vip'){?>class="active"<?php }?> ><a href="?pd=vip">VIP记录</a></li>
				<li <?php if(isset($_GET["pd"]) && $_GET["pd"]=='recharge'){?>class="active"<?php }?> ><a href="?pd=recharge" >充值记录</a></li>
				<li <?php if(isset($_GET["pd"]) && $_GET["pd"]=='ref'){?>class="active"<?php }?> ><a href="?pd=ref">推广注册</a></li>
				<li <?php if(isset($_GET["pd"]) && $_GET["pd"]=='ref2'){?>class="active"<?php }?> ><a href="?pd=ref2">推广VIP</a></li>
				<li <?php if(isset($_GET["pd"]) && ($_GET["pd"]=='outmo' || $_GET["pd"]=='tixian')){?>class="active"<?php }?> ><a href="?pd=outmo">站内提现</a></li>
				<li <?php if(isset($_GET["pd"]) && $_GET["pd"]=='pass'){?>class="active"<?php }?> ><a href="?pd=pass">修改密码</a></li>
		...
		<div class="pagecontent profile-content">
			...
			<?php }elseif(isset($_GET["pd"]) && $_GET["pd"]=='ref2'){
			...
			?>
				<div class="profile-form">
					...
					<dl class="dl-horizontal">
						<dt>支付宝账号</dt>
						<dd><input type="text" class="profile-input" id="ice_alipay" name="ice_alipay" value="<?php if($userAli) echo $userAli->ice_alipay;?>"/></dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>支付宝姓名</dt>
						<dd><input type="text" class="profile-input" id="ice_name" name="ice_name"  value="<?php if($userAli) echo $userAli->ice_name;?>"/></dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>手续费</dt>
						<dd>
					...
					<dl class="dl-horizontal">
						<dt>提现金额</dt>
						<dd>
							<input type="text" class="profile-input" id="ice_money" name="ice_money"  />（余额：<?php echo sprintf("%.2f",$okMoney)?><?php echo get_option('ice_name_alipay')?>）
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt></dt>
			 ...
			 <?php }elseif(isset($_GET["pd"]) && $_GET["pd"]=='pass'){?>
			...
			<div id="changepass">
				...
				<div class="profile-form">
                    ...
                    <dl class="dl-horizontal" style="display:none">
						<dt>用户名</dt>
						<dd>
							<input type="text" class="profile-input" id="mm_username" name="log" value="<?php echo esc_attr( $profileuser->user_login ); ?>">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>新密码</dt>
						<dd>
							<input type="password" class="profile-input" id="mm_pass_new" name="mm_pass_new" value="">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>重复密码</dt>
						<dd>
							<input type="password" class="profile-input" id="mm_pass_new2" name="mm_pass_new2" value="">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt></dt>
						...
						<?php
							if($list) {
								foreach($list as $value)
								{
									...
									echo "<td>$value->ice_price</td>\n";
									echo "<td>$value->ice_time</td>\n";
									echo "<td><a href='".get_bloginfo('wpurl').'/wp-content/plugins/erphpdown/download.php?url='.$value->ice_url."' target='_blank'>下载</a></td>\n";
									echo "</tr>";
								}
							}
							else
			...
			<?php }elseif(isset($_GET["pd"]) && $_GET["pd"]=='money'){ 
					$user_Info   = wp_get_current_user();
					$userMoney=$wpdb->get_row("select * from ".$wpdb->iceinfo." where ice_user_id=".$user_Info->ID);
					if(!$userMoney){
						$okMoney=0;
				...
				<div class="profile-box">
                    ...
                    消费<?php echo get_option('ice_name_alipay');?>：<?php echo $userMoney?sprintf("%.2f",$userMoney->ice_get_money):0;?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;剩余<?php echo get_option('ice_name_alipay');?>：<?php echo sprintf("%.2f",$okMoney);?><?php if(plugin_check_cred() && get_option('erphp_mycred') == 'yes'){$mycred_core = get_option('mycred_pref_core');?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;剩余<?php echo $mycred_core['name']['plural'];?>：<?php echo mycred_get_users_cred( $user_Info->ID )?><?php }?>
				</div>
                ...
                <form action="" method="post" onSubmit="return checkFm();">
                        ...
                        <table class="form-table">
                            <tr>
                				<td>
                				<input type="text" class="profile-input" id="ice_money" name="ice_money" required="required"/>（请输入充值金额，1人民币 = <?php echo get_option('ice_proportion_alipay').' '.get_option('ice_name_alipay')?>）
                				</td>
                            </tr>
                            <tr>
                                <td>
			            		...
			            		?>
									...
									<?php if(get_option('erphpdown_payjs_appid')){?>  
										<?php if(!get_option('erphpdown_payjs_alipay')){?><input type="radio" id="paytype20" class="paytype" name="paytype" value="20" checked />支付宝&nbsp;<?php }?>
										<?php if(!get_option('erphpdown_payjs_wxpay')){?><input type="radio" id="paytype19" class="paytype" name="paytype" value="19" checked />微信&nbsp;<?php }?>    
									<?php }?>
					                ...
					                <?php if(get_option('erphpdown_codepay_appid')){?> 
					                	<?php if(!get_option('erphpdown_codepay_alipay')){?><input type="radio" id="paytype13" class="paytype" name="paytype" value="13" checked />支付宝&nbsp;<?php }?>
					                	<?php if(!get_option('erphpdown_codepay_wxpay')){?><input type="radio" id="paytype14" class="paytype" name="paytype" value="14" />微信&nbsp;<?php }?>
					                	<?php if(!get_option('erphpdown_codepay_qqpay')){?><input type="radio" id="paytype15" class="paytype" name="paytype" value="15" />QQ钱包&nbsp;<?php }?>        
					                <?php }?>
					                <?php if(get_option('erphpdown_paypy_key')){?> 
										...
										<?php if(!get_option('erphpdown_paypy_alipay')){?><input type="radio" id="paytype8" class="paytype" name="paytype" value="8" checked/>支付宝&nbsp;<?php }?>  
									<?php }?> 
									<?php if(get_option('erphpdown_epay_id')){?>
										<?php if(!get_option('erphpdown_epay_alipay')){?><input type="radio" id="paytype21" class="paytype" name="paytype" value="21" checked />支付宝&nbsp;<?php }?>
										<?php if(!get_option('erphpdown_epay_wxpay')){?><input type="radio" id="paytype22" class="paytype" name="paytype" value="22" checked/>微信&nbsp;<?php }?>
									<?php }?>
									<?php if(get_option('erphpdown_vpay_key')){?>
										<?php if(!get_option('erphpdown_vpay_alipay')){?><input type="radio" id="paytype31" class="paytype" name="paytype" value="31" checked />支付宝&nbsp;<?php }?>
										<?php if(!get_option('erphpdown_vpay_wxpay')){?><input type="radio" id="paytype32" class="paytype" name="paytype" value="32" checked/>微信<?php }?>
									<?php }?>
	                            <?php }?>
                                </td>
                            </tr>
                ...
                <div class="profile-box">
                	<form action="" method="post">
                        <h2 class="profile-title">VIP充值卡</h2>
                        <table class="form-table">
                            <tr>
                				<td>
                				VIP卡号：<input type="text" id="epdcardnum" name="epdcardnum"  required="required" />
                				</td>
                            </tr>
                ...
                <h2 class="profile-title">升级VIP</h2>
                	<table class="form-table">
                        <tr>
                            ...
                            if($userTypeId==6)
                            {
                                echo "您目前是".$erphp_day_name;
                            }
                            elseif($userTypeId==7)
                            {
                                echo "您目前是".$erphp_month_name;
                            }
                            elseif ($userTypeId==8)
                            {
                                echo "您目前是".$erphp_quarter_name;
                            }
                            elseif ($userTypeId==9)
                            {
                                echo "您目前是".$erphp_year_name;
                            }
							elseif ($userTypeId==10)
                            {
                                echo "您目前是".$erphp_life_name;
                            }
                            else 
                            {
                                echo '您未购买任何VIP服务';
                            }
                            ?>&nbsp;&nbsp;&nbsp;<?php echo (($userTypeId>0 && $userTypeId<10) || ($userTypeId == 10 && get_option('erphp_life_days'))) ?'到期时间：'.getUsreMemberTypeEndTime() :'';?><br /></td>
                        </tr>
                        <tr>
                            <td>类型：
                                <?php if($ciphp_life_price){?><input type="radio" id="userType10" name="userType" value="10" checked /><?php echo $erphp_life_name;?>(<?php echo $ciphp_life_price.get_option('ice_name_alipay');if($erphp_life_days) echo '&nbsp;'.$erphp_life_days.'年';?>)<?php }?>&nbsp;
                                <?php if($ciphp_year_price){?><input type="radio" id="userType9" name="userType" value="9" checked/><?php echo $erphp_year_name;?>(<?php echo $ciphp_year_price.get_option('ice_name_alipay');if($erphp_year_days) echo '&nbsp;'.$erphp_year_days.'个月';?>)&nbsp;<?php }?>
                                <?php if($ciphp_quarter_price){?><input type="radio" id="userType8" name="userType" value="8" checked/><?php echo $erphp_quarter_name;?>(<?php echo $ciphp_quarter_price.get_option('ice_name_alipay');if($erphp_quarter_days) echo '&nbsp;'.$erphp_quarter_days.'个月';?>)&nbsp;<?php }?>
                                <?php if($ciphp_month_price){?><input type="radio" id="userType7" name="userType" value="7" checked/><?php echo $erphp_month_name;?>(<?php echo $ciphp_month_price.get_option('ice_name_alipay');if($erphp_month_days) echo '&nbsp;'.$erphp_month_days.'天';?>)&nbsp;<?php }?>
                                <?php if($ciphp_day_price){?><input type="radio" id="userType6" name="userType" value="6" checked/><?php echo $erphp_day_name;?>(<?php echo $ciphp_day_price.get_option('ice_name_alipay');if($erphp_day_days) echo '&nbsp;'.$erphp_day_days.'天';?>)<?php }?>
                            </td>
                        </tr>
                        <tr>
                            <td>方式：
                        ...
                        <tr>
                            <td>
                            	<p class="submit">
	                            	<input type="submit" value="支付升级" class="profile-btn" name="erphp-pay-vip">
	                            </p>
                            </td>
                        </tr>
                ...
                <form action="" method="post">
                	<table class="form-table">
                        ...
                        <tr>
                            <td>
                            	<p class="submit">
	                            	<input type="submit" name="Submit" value="余额升级" class="profile-btn" onClick="return confirm('确认升级成为VIP?');"/>
	                            </p>
                            </td>
                        </tr>
