...
function readers_wall( $outer='1', $timer='3', $limit='100' ){
    ...
    foreach ($sql as $comment) {
        ...
        if( $i < 4 ){
            $html .= '<a class="item-top item-'.$i.'" target="_blank" href="'. $c_url . '"><h4>【'.$tt.'】<small><i>评论：</i>('. $comment->cnt . ')</small></h4>'.str_replace(' src=', ' data-src=', _get_avatar( $user_id=$comment->user_id, $user_email=$comment->comment_author_email) ).'<span><strong>'.$comment->comment_author.'</strong><b>'.$c_url.'</b></span></a>';
        }else{
            $html .= '<a target="_blank" href="'. $c_url . '" title="【'.$tt.'】评论：'. $comment->cnt . '">'.str_replace(' src=', ' data-src=', _get_avatar($user_id=$comment->user_id, $user_email=$comment->comment_author_email)).'<span><strong>'.$comment->comment_author.'</strong></span></a>';
        }
