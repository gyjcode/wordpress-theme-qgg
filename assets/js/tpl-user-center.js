    ...
    return {
        init: function (){
            ...
            var _main = $('.user-main'),
                ...
                ajax_url = GSM.uri + '/action/user_center.php',
            ...
            $('#user-center').on('click', function(event){
                ...
                switch( type ){
                    ...
                    case 'userinfo.submit':
                        ...
                        if( inputs.url && (!GSM.is_url(inputs.url) || inputs.url.length>100) ){
                            tips('网址格式错误,注意以http开头,后面不要加反斜杠/');
                            return;
                        }
                        if( inputs.qq && !GSM.is_qq(inputs.qq) ){
                            tips('QQ格式错误');
                            return;
                        }
                        ...
                        if( inputs.weibo && (!GSM.is_url(inputs.weibo) || inputs.weibo.length>100) ){
                            tips('微博格式错误');
                            return;
                        }
