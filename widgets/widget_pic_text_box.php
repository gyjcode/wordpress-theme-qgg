...
?>
<?php
class widget_pic_text_box extends WP_Widget {

	function __construct(){
		parent::__construct( 'widget_pic_text_box', __('QGG 图文盒子', 'QGG'), array( 'classname' => 'widget-pic-text-box', 'description'=> '图文标题、图文描述、详情按钮、特色图片、跳转链接' ) );
	}
	...
	function form($instance) {
		$defaults = array( 
			...
			'desc'    => '分享网站建设中遇到的WordPress、Linux，Apache，Nginx，PHP，HTML，CSS等的问题及解决方案；
						分享Windows操作系统及其周边的一些经验知识；分享互联网使用过程中遇到的一些问题及其处理技巧；
						分享一些自己在读书过程中的心得体会；分享一些自己觉得有意义的音视频内容 ... ...', 
			'button'  => '了解详情',
			'link'    => 'https://blog.quietguoguo.com/', 
			'imglink' => get_template_directory_uri() . '/img/thumbnail.png', 
			'blank'   => 'on'
		);
	...
	function widget( $args, $instance ) {
		...
		echo '<div class="content"><div>';
			echo '<h3>'.$title.'</h3>';
			echo '<p>'.$desc.'</p>';
			echo '<a href="'.$link.'"'.$lank.'>'.$button.'</a>';
		echo '</div></div>';
		echo $after_widget;
	}
}