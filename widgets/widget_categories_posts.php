...
?>
<?php
class widget_categories_posts extends WP_Widget {
	
	function __construct(){
		parent::__construct( 'widget_categories_posts', __('QGG 分类文章', 'QGG'), array( 'classname' => 'widget-categories-posts', 'description'=> '分类限制、排序方式、文章数目、显示图片、显示评论' ) );
	}
	...
	function widget( $args, $instance ) {
		...
		$style       = '';
		if( !$show_thumb ) $style = ' class="nopic"';
		...
		echo '<ul'.$style.'>';
		echo categories_posts_list( $orderby, $post_num, $cats_limit, $show_thumb, $show_author, $show_comts, $show_views, $show_likes );
		echo '</ul>';
		echo $after_widget;
	}
}
		...
		<a <?php the_post_target_blank(); ?> href="<?php the_permalink(); ?>">
			...
			<div class="desc">
				...
				<div class="muted">
					...
					<?php } ?>
					<?php if( !QGG_Options('site_comment_closed_open') && $show_comts ){ ?>
					<span class="comts"><?php echo '评论&nbsp(', comments_number('0', '1', '%'), ')'; ?></span>
					<?php } ?>
					<?php if( $show_views ){ ?>
					<span class="views"><?php echo '阅读&nbsp(', _get_post_views(), ')'; ?></span>
					<?php } ?>
					<?php if( $show_likes ){ ?>
					<span class="likes"><?php echo '喜欢&nbsp(', _get_post_likes(), ')'; ?></span>
					<?php } ?>
				</div>
			</div>
		</a>
