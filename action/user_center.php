...
/**
 * @name 会员信息修改提交
 * @description 会员中心处理用户提交数据（报考投稿文章、密码修改、信息修改等）
 */
?>
...
if( !$_POST ){ exit; }
// 会员中心未开启时直接退出
if( !QGG_Options('user_center_open') ){ exit; }
// 用户未登录时直接退出
...
switch ($user_input['action']) {
	...
	case 'publish':
		// 投稿功能未开启时直接退出
		if( !QGG_Options('user_publish_open') ){
		...
		// 邮件提醒站长投稿文章信息
		if( QGG_Options('user_publish_mail_send') ){
			wp_mail(QGG_Options('user_publish_mail_to'), '站长，有新投稿：'.$post_title, $post_content);
		}
		// 投稿成功,返回信息
	...
	case 'info.edit':
		...
		// 校验用户昵称是否为空
		if( !$user_input['nickname'] || ($user_input['nickname'] && get_string_length($user_input['nickname'])>12) || ($user_input['nickname'] && get_string_length($user_input['nickname'])<2) ) {  
		...
		// 校验用户网址是否合规
		if( $user_input['url'] && (!preg_match("/^((http|https)\:\/\/)([a-z0-9-]{1,}.)?[a-z0-9-]{2,}.([a-z0-9-]{1,}.)?[a-z0-9]{2,}$/", $user_input['url']) || get_string_length($user_input['url'])>100) ){
		...
		//校验微信账号是否合规
		if( $user_input['wechat'] && get_string_length($user_input['wechat'])>30 ) {  
			print_r(json_encode(array('error'=>1, 'msg'=>'微信字数过长，限制在30字内')));
			exit();  
		}
		// 校验微博账号是否合规
		if( $user_input['weibo'] && (!preg_match("/^((http|https)\:\/\/)([a-z0-9-]{1,}.)?[a-z0-9-]{2,}.([a-z0-9-]{1,}.)?[a-z0-9]{2,}$/", $user_input['weibo']) || get_string_length($user_input['weibo'])>100) ){
...
function user_post_data(){
	...
	while ( have_posts() ) : the_post(); 
		...
		$items[] = array(
			...
			'like'    => user_get_post_likes_number(),
		);
...
function user_get_thumbnail_src() {  
	global $post;
	$thumb = _get_post_thumbnail();  
	preg_match_all('/<img.*?(?: |\\t|\\r|\\n)?src=[\'"]?(.+?)[\'"]?(?:(?: |\\t|\\r|\\n)+.*?)?>/sim', $thumb, $strResult, PREG_PATTERN_ORDER);  
	return $strResult[1];
}
// 获取文章喜欢数
function user_get_post_likes_number(){
	global $post;
	$post_ID = $post->ID;
